...
params = {
  'Old Faithful': {'interval_min': 62, 'interval_maj': 95, 'window' : 10, 'ie_interval_adjustment': 2, 'ie_window_adj': 1},
  'Daisy': {'interval_min': 2.75 * 60, 'interval_maj': 2.75 * 60, 'window' : 15, 'ie_interval_adjustment': 2, 'ie_window_adj': 1},
  #'Castle': {'interval_min': 62, 'interval_maj': 95, 'window' : 10, 'ie_interval_adjustment': 2, 'ie_window_adj': 1},
  'Riverside': {'interval_min': 6 * 60, 'interval_maj': 6 * 60, 'window' : 30, 'ie_interval_adjustment': 15, 'ie_window_adj': 10},
  'Grand': {'interval_min': 7.5 * 60, 'interval_maj': 7.5 * 60, 'window' : 1.5 * 60, 'ie_interval_adjustment': 5, 'ie_window_adj': 10}
  }
...
def predict(geyser,entry):
    ...
    print "Send to GeyserTimes.org: {'GeyserID': %s, 'prediction': %s, 'window': %s}" % (entry['geyserID'], prediction + tz_offset , window)
